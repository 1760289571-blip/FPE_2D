...
def simulate_2d(params):
    ...
    for k in range(steps):
        ...
        v[v >= params.v_thres] = params.v_reset
...
def make_plots_4d(samples, params):
...
def make_plots_2d(samples, params):
    ...
    fig.suptitle(
        f"2D SDE validation (tau_r=0): mu_ex={params.mu_ex}, sigma_ex={params.sigma_ex}",
        fontsize=12,
    )
    fig.tight_layout()
...
def check_steady_state_operator_2d(samples, params):
    ranges = [(params.v_min, params.v_max), (params.g_min, params.g_max)]
    bins = [params.grid_bins] * 2
    hist, edges = np.histogramdd(samples, bins=bins, range=ranges, density=False)
    dv = edges[0][1] - edges[0][0]
    dg = edges[1][1] - edges[1][0]
    cell_volume = dv * dg
    P = hist / (np.sum(hist) * cell_volume + 1e-16)

    vc = 0.5 * (edges[0][1:] + edges[0][:-1])
    gc = 0.5 * (edges[1][1:] + edges[1][:-1])
    V, G = np.meshgrid(vc, gc, indexing='ij')

    A_v = (-V + params.mu_ex + G) / params.tau_m
    A_g = (-G + params.s_ee * params.mu_ffd * params.p_ee) / params.tau_ee
    Dv = 0.5 * (params.sigma_ex / params.tau_m) ** 2
    Dg = 0.5 * (params.sigma_ffd * params.s_ee * np.sqrt(params.p_ee) / params.tau_ee) ** 2

    LP = -_safe_grad(A_v * P, dv, 0) - _safe_grad(A_g * P, dg, 1)
    LP += Dv * _safe_hess(P, dv, 0) + Dg * _safe_hess(P, dg, 1)

    RP = np.zeros_like(P)
    if params.approx_reset and params.tau_r == 0.0:
        i_th = np.searchsorted(vc, params.v_thres)
        i_r = np.argmin(np.abs(vc - params.v_reset))
        i_th = np.clip(i_th, 1, len(vc) - 2)
        dP_dv = _safe_grad(P, dv, 0)
        flux = np.maximum(0.0, -Dv * dP_dv[i_th, :])
        RP[i_r, :] += flux / max(dv, 1e-12)

    total = LP + RP
    boundary_mask = (np.abs(V - params.v_thres) < params.exclude_width) | (np.abs(V - params.v_reset) < params.exclude_width)
    interior = ~boundary_mask

    l1_abs = np.mean(np.abs(total[interior]))
    linf_abs = np.max(np.abs(total[interior]))
    pref = np.mean(np.abs(P[interior])) + 1e-14
    print("\n--- 2D steady-state check: (L+R)P ≈ 0 ---")
    print(f"Grid bins per dim: {params.grid_bins}")
    print(f"Interior points used: {interior.sum()} / {interior.size}")
    print(f"mean(|(L+R)P|): {l1_abs:.4e}")
    print(f"max(|(L+R)P|):  {linf_abs:.4e}")
...
def compare_to_reference_2d(sim_density, params):
    ...
    print("\n--- 2D validation metrics (simulation vs theoretical/reference) ---")
...
def build_argparser():
    ...
    p.add_argument('--mode', type=str, default='2d', choices=['2d', '4d'])
...
def main():
    ...
    if args.mode == '4d':
        samples = simulate_4d(args)
        make_plots_4d(samples, args)
        save_4d_density(samples, args)
        if args.check_steady:
            check_steady_state_operator(samples, args)
    else:
        samples = simulate_2d(args)
        make_plots_2d(samples, args)
        ranges = [(args.density_v_min, args.density_v_max), (args.density_g_min, args.density_g_max)]
        bins = [args.density_bins] * 2
        sim_density, _ = np.histogramdd(samples, bins=bins, range=ranges, density=True)
        save_2d_density(samples, args)
        compare_to_reference_2d(sim_density, args)
        if args.check_steady:
            check_steady_state_operator_2d(samples, args)
