...
parser.add_argument('--mu_ex', type = float, default = 5)
...
Loss function
'''
Loss_Params = {"reg_bound": 1e2,"reg_diff": 1.0, "penal_param":1e1}
...
r_plot1 = args.r_sde  # 绘图范围，比训练范围小一些以聚焦重要区域
r_plot2 = args.r_sde*0.5
# 创建评估网格
...
print(f"归一化后积分: {np.sum(Z_normalized) * dz * dr:.6f}")
'''
# 创建热力图
print("生成热力图...")
...
nn_density_save_path = os.path.join(Save_Path, "nn_density.npz")
np.savez_compressed(
