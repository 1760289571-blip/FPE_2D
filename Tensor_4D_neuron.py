...
def KDE_no_bp(param, data):
    ...
    KDE_derivative_v = ((output_KDE_grad[:,0:2]).prod(axis = -1)) * (output_KDE[:, 2:4].prod(axis = -1)) * Factor_Loss
    KDE_derivative_g = ((output_KDE_grad[:,2:4]).prod(axis = -1)) * (output_KDE[:, 0:2].prod(axis = -1)) * Factor_Loss
...
Loss function
'''
Loss_Params = {"reg_bound": 5e0,"reg_diff": 1.0, "penal_param":1e5}
...
# 设置绘图参数
grid_size = 50  # 网格大小，可调整
r_plot1 = 19.6  # 绘图范围，比训练范围小一些以聚焦重要区域
r_plot2 = 9.8
# 创建评估网格
...
y = np.linspace(-r_plot1, r_plot1, grid_size)
X, Y = np.meshgrid(x, y)
Z=np.full_like(X,0)
R=np.full_like(X,0)
...
'''
'''
z = np.linspace(-r_plot, r_plot, grid_size)
r = np.linspace(-r_plot, r_plot, grid_size)
Z, R = np.meshgrid(z, r)
X=np.full_like(Z,0)
Y=np.full_like(Z,0)
grid_points = np.column_stack([X.ravel(), Y.ravel(),Z.ravel(),R.ravel()])
...
a = np.linspace(-r_plot, r_plot, grid_size)
X, A = np.meshgrid(x, z)
Y=np.full_like(X,0)
B=np.full_like(X,0)
grid_points = np.column_stack([X.ravel(), Y.ravel(),A.ravel(),B.ravel()])
'''


x = np.linspace(-r_plot1, r_plot1, grid_size)
y = np.linspace(-r_plot1, r_plot1, grid_size)
z = np.linspace(-r_plot2, r_plot2, grid_size)
r = np.linspace(-r_plot2, r_plot2, grid_size)
X, Y, Z, R = np.meshgrid(x, y, z, r)
grid_points = np.column_stack([X.ravel(), Y.ravel(),Z.ravel(),R.ravel()])
...
integral = np.sum(Z) *  dx * dy * dz * dr
Z_normalized = Z / integral

print(f"¸概率密度积分: {integral:.6f}")
print(f"归一化后积分: {np.sum(Z_normalized) * dx * dy * dz * dr:.6f}")
...
# 1. 标准热力图
ax1 = axes[0, 0]
im1 = ax1.imshow(Z_normalized, extent=[-r_plot1, r_plot1, -r_plot1, r_plot1], 
