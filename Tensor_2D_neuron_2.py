...
def KDE_no_bp(param, data):
    ...
    '''
    fun_d_v = -func_into(data,I,tau_m)
    ...
    for i in range(dim):
        ...
        Lp = jnp.square(sigma)/(2*jnp.square(tau_m))*Laplacian_value + Lp
        
    #calculate diffusion term (non-diagonal term)
    KDE_derivative_choose = ((output_KDE_grad[:, :]).prod(axis = -1)) * Factor_Loss
    
    Lp = c*jnp.square(sigma/tau_m) *  ( ( KDE_derivative_choose * coeff )).sum()   + Lp
...
r_plot = args.r_sde  # 绘图范围，比训练范围小一些以聚焦重要区域

# 创建评估网格
print(f"创建 {grid_size}x{grid_size} 评估网格...")
...
# 使用tqdm显示进度、
for i in tqdm.tqdm(range(0, len(grid_points), batch_size), desc="评估进度"):
...
fig.suptitle(f'稳态概率密度分布 - 参数: I={args.I}, τ_m={args.tau_m}, σ={args.sigma}, c={args.c}', 
