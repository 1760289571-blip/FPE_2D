...
parser.add_argument('--sigma_ffd', type = float, default = 0.8)
...
def func_into(data, mu_ex, mu_ffd, tau_m, tau_ee, p_ee, s_ee):
    return jnp.asarray([
        (-data[0] + mu_ex + data[1]) / tau_m,
        (-data[1] + s_ee * mu_ffd * p_ee) / tau_ee,
    ])
'''
Part 2
...
def Initialization_Generation(key, width_ini, dist_ini_shift, constraint_ini_shift, shape):
    ...
    return Param_Ini
def init_q_param(key):
    param = Initialization_Generation(
        key,
        0.9 * r,
        jnp.sqrt(r),
        r,
    ...
    key_c = jax.random.PRNGKey(9876)
    param["coeff"] = 0.01 * jax.random.normal(key_c, shape=(Shape[2],))
...
scale_bound = {"wendland": 3e-02,"inverse_quadratic": 0.01,'gaussian': 0.01, 'truncated_gaussian': 0.01}
...
def truncated_gaussian_integral(sigma, shift):
    sigma_t =  1/ (scale_bound["gaussian"] + jnp.square(sigma) )
    delta = r - abs(shift)
    d=jnp.sqrt(sigma_t/(2 * jnp.pi))*jnp.exp(-0.5 * sigma_t * jnp.square(delta))
    results = jax.lax.erf(delta*jnp.sqrt(sigma_t/2))-2*delta*d
    return results
...
def truncated_gaussian_1_test(sigma, data, shift):

    input = data - shift
    Integral_scaling = truncated_gaussian_integral(sigma, shift)
    sigma_t =  1/ (scale_bound["gaussian"] + jnp.square(sigma) )
    delta = r - abs(shift)
    d=jnp.sqrt(sigma_t/(2 * np.pi))*jnp.exp(-0.5 * sigma_t * jnp.square(delta))
    dim_each_result = (1/Integral_scaling) * jax.nn.relu(( jnp.sqrt(sigma_t)/jnp.sqrt(2 * np.pi) ) * jnp.exp( -0.5 * sigma_t * jnp.square(input))-d)
...
def truncated_gaussian_1(sigma, data, shift):
    input = data - shift
    Integral_scaling = truncated_gaussian_integral(sigma, shift)

    sigma_t =  1/ (scale_bound["gaussian"] + jnp.square(sigma) )
    temp_each_result = jnp.sqrt(sigma_t)/jnp.sqrt(2 * np.pi) * jnp.exp( -0.5 * sigma_t * jnp.square(input))
    delta = r - abs(shift)
    d=jnp.sqrt(sigma_t/(2 * np.pi))*jnp.exp(-0.5 * sigma_t * jnp.square(delta))
    dim_each_result = (1/Integral_scaling) * jax.nn.relu(temp_each_result-d)
    #grad_dim_each_result = (1/Integral_scaling) * (-sigma_t * input * dim_each_result)
    #hessian_dim_each_result = (1/Integral_scaling) * (jnp.square(sigma_t) * jnp.square(input) - sigma_t) * dim_each_result
    mask = (temp_each_result > d)
    grad_dim_each_result =  (1/Integral_scaling) * jnp.where(mask,-sigma_t * input * temp_each_result,0.0)
    hessian_dim_each_result = (1/Integral_scaling) * jnp.where(mask, (jnp.square(sigma_t) * jnp.square(input) - sigma_t) * temp_each_result,0.0)
    return dim_each_result, grad_dim_each_result, hessian_dim_each_result
...
Kernel_Dic = {
    ...
    "truncated_gaussian": (truncated_gaussian_integral, truncated_gaussian_1_test, truncated_gaussian_1)
}
...
def KDE_no_bp(param, data):
    lp, rp = KDE_operator_parts(param, data)
    return lp + rp

def KDE_operator_parts_q(param, data):
    alpha_1 = jnp.square(param['alpha_1'])/(jnp.square(param['alpha_1'])).sum(axis = 0)
    alpha_2 = jnp.square(param['alpha_2']) / (jnp.square(param['alpha_2'])).sum(axis = 0)
    coeff = param["coeff"]
...
def KDE_operator_parts(param, data):
    alpha_1 = jnp.square(param['alpha_1'])/(jnp.square(param['alpha_1'])).sum(axis = 0)
    alpha_2 = jnp.square(param['alpha_2']) / (jnp.square(param['alpha_2'])).sum(axis = 0)
    coeff = jnp.square(param["coeff"]) /(jnp.square(param["coeff"])).sum()
...
vec_KDE_q = jit( vmap(KDE_q, in_axes = (None, 0) , out_axes = 0) )
...
def penalty_constraint_param(param):
    #r_j - |s_{ij}^{(\ell)} - O_j|
    eps=0.1
    r_1_constraint_result = jnp.abs(param["shifts"] - Center_O) - r+eps
    ...
    Penalty = shifts_constraint_penalty
...
Loss function
'''
Loss_Params = {"reg_bound": 5e5,"reg_diff": 1.0, "penal_param":1e5}
...
def batch_result(data, batch_size, param):
    ...
    return KDE_ESTIMATE/Factor_Loss
...
def run_section_2_3_q_solver(p0_param, save_dir):
    ...
    if os.path.exists(summary_path):
        summary_npz = np.load(summary_path, allow_pickle=True)
        if "theoretical_r0" in summary_npz:
            r0 = float(summary_npz["theoretical_r0"])
        else:
            _, _, _, r0 = compute_theoretical_firing_rate(p0_param, -10.0, 10.0, args.firing_rate_num_g)
    else:
        _, _, _, r0 = compute_theoretical_firing_rate(p0_param, -10.0, 10.0, args.firing_rate_num_g)

    omega = float(args.omega)
    omega_eps = np.sign(omega) * max(abs(omega), 1e-6)

    def uv_value(param_uv, data):
        return KDE_q(param_uv["U"], data), KDE(param_uv["V"], data)

    def uv_residual_single(param_uv, data):
        lu, ru = KDE_operator_parts_q(param_uv["U"], data)
        lv, rv = KDE_operator_parts_q(param_uv["V"], data)
        p0_value = KDE(p0_param, data)
        _, rp0 = KDE_operator_parts(p0_param, data)
        u_val, v_val = uv_value(param_uv, data)

        cwt = jnp.cos(omega * tau_r)
        swt = jnp.sin(omega * tau_r)
        rhs_coeff_real = swt / omega_eps - cwt / r0
        rhs_coeff_imag = (1.0 - cwt) / omega_eps - swt / r0

        lhs_real = lu + cwt * ru - omega * v_val - swt * rv
        lhs_imag = lv + cwt * rv + omega * u_val + swt * ru
        rhs_real = p0_value + rhs_coeff_real * rp0
        rhs_imag = rhs_coeff_imag * rp0
        return lhs_real - rhs_real, lhs_imag - rhs_imag

    vec_uv_residual = jit(vmap(uv_residual_single, in_axes=(None, 0), out_axes=(0, 0)))

    def q_loss(param_uv, batch):
        rr, ri = vec_uv_residual(param_uv, batch)
        pde_term = (jnp.square(rr) + jnp.square(ri)).mean()
        reg_term = (
            args.q_reg_bound * (Boundary_Control(param_uv["U"]) + Boundary_Control(param_uv["V"]))
            + args.q_reg_param * (penalty_constraint_param(param_uv["U"]) + penalty_constraint_param(param_uv["V"]))
        )
        return pde_term + reg_term

    def q_resample(i):
        key = jax.random.PRNGKey(seeds[i])
        return jax.random.uniform(key, minval=Center_O - r, maxval=Center_O + r, shape=((args.q_batches, dim)))

    init_uv = {
    "U": init_q_param(jax.random.PRNGKey(111)),
    "V": init_q_param(jax.random.PRNGKey(222)),
}
    optimizer = optax.chain(optax.clip(100.0), optax.adam(learning_rate=args.q_lr))
    opt_state = optimizer.init(init_uv)

    @jit
    def step(param_uv, state, batch):
        loss_val, grads = jax.value_and_grad(q_loss)(param_uv, batch)
        updates, state = optimizer.update(grads, state, param_uv)
        param_uv = optax.apply_updates(param_uv, updates)
        return loss_val, param_uv, state

    q_losses = []
    for epoch in tqdm.tqdm(range(args.q_epochs)):
        batch = q_resample(epoch)
        loss_val, init_uv, opt_state = step(init_uv, opt_state, batch)
        q_losses.append(float(loss_val))
        if epoch % 1000 == 0:
            print(f"[Q solver] epoch={epoch}, loss={float(loss_val):.6e}")

    q_param_path = os.path.join(save_dir, args.q_param_name)
    ...
    q_points = np.column_stack((Xq.ravel(), Gq.ravel()))
    u_eval = vec_KDE_q(init_uv["U"], q_points).reshape(Xq.shape) / Factor_Loss
...
if args.task_mode == "q_omega":
   ...
   if not os.path.exists(p0_param_source):
       raise FileNotFoundError(f"P0 parameter file not found: {p0_param_source}. Please run task_mode='p0' first.")
   Final_Param = pickle.load(open(p0_param_source, "rb"))
   run_section_2_3_q_solver(Final_Param, Save_Path)
   sys.exit(0)
else:
   if args.Train == True:
...
r_plot1 = 20.0  # 绘图范围，比训练范围小一些以聚焦重要区域
r_plot2 = 10.0
# 创建评估网格
print(f"′′?¨ {grid_size}x{grid_size} ?à1àí???...")

x = np.linspace(-r_plot1, r_plot1, grid_size)
y = np.linspace(-r_plot2, r_plot2, grid_size)
X, Y = np.meshgrid(x, y)
...
# 使用tqdm显示进度
for i in tqdm.tqdm(range(0, len(grid_points), batch_size), desc="?à1à???è"):
...
fig.suptitle(f'steady state Prob distribution - param: I={args.mu_ex}, tau_m={args.tau_m}, sigma={args.sigma_ex}, c={args.c}', 
             fontsize=16, fontweight='bold')

# 1. 标准热力图
ax1 = axes[0, 0]
im1 = ax1.imshow(Z_normalized, extent=[-r_plot1, r_plot1, -r_plot2, r_plot2], 
                origin='lower', cmap='hot', aspect='auto')
ax1.set_xlabel('v', fontsize=12)
ax1.set_ylabel('g', fontsize=12)
ax1.set_title('P0', fontsize=14)
ax1.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
plt.colorbar(im1, ax=ax1, label='Prob Density')
